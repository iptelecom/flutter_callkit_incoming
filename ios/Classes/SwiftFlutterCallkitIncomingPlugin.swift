...
public class SwiftFlutterCallkitIncomingPlugin: NSObject, FlutterPlugin, CXProviderDelegate {
    ...
    static let ACTION_CALL_TOGGLE_DTMF = "com.hiennv.flutter_callkit_incoming.ACTION_CALL_TOGGLE_DTMF"
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        print("Swift: handle: \(call.method)")
        switch call.method {
        ...
        case "setHold":
            ...
            if let getArgs = args as? [String: Any] {
                self.data = Data(args: getArgs)
                print(self.data ?? "No Data")
                self.setHold(self.data!, onHold: getArgs["onHold"]! as? Bool ?? false)
            }
            result("OK")
            break
        case "muteCall":
    ...
    @objc public func startCall(_ data: Data, fromPushKit: Bool) {
        ...
        if (uuid != nil) {
            call = self.callManager?.callWithUUID(uuid: uuid!)
        }
...
//            }
//            self.sendEvent(SwiftFlutterCallkitIncomingPlugin.ACTION_CALL_ENDED, data.toJSON())
////             self.sendEvent(SwiftFlutterCallkitIncomingPlugin.ACTION_CALL_TOGGLE_MUTE, data.toJSON())
//        } else {
            uuid = UUID(uuidString: data.uuid)
            if (uuid != nil) {
                call = self.callManager?.callWithUUID(uuid: uuid!)
...
//        }
    ...
    @objc public func setMute(_ data: Data, muted: Bool) {
        var call: Call? = nil
        var uuid: UUID? = nil
        print("Swift: setMute")
        print(muted)
...
//         }
        ...
        print(self.callManager?.calls.map({ $0.uuid })  ?? "No Calls!")
        if (call != nil) {
            self.callManager?.setMute(call: call!, muted: muted)
        }
    }

    @objc public func setHold(_ data: Data, onHold: Bool) {
        var call: Call? = nil
        var uuid: UUID? = nil
        print("Swift: setHold")
        print(data.toJSON())
...
//         }
    ...
    @objc public func setAnswered(_ data: Data) {
        var call: Call? = nil
        var uuid: UUID? = nil
        print("Swift: setMute")
        print(data.toJSON())
...
//         }
        ...
        print(self.callManager?.calls.map({ $0.uuid })  ?? "No Calls!")
        if (call != nil) {
            self.callManager?.setAnswered(call: call!)
        }
    }

    
    @obj public func activeCalls() -> [[String: Any?]] {
    ...
    func createConfiguration(_ data: Data) -> CXProviderConfiguration {
        ...
        configuration.supportsVideo = data.supportsVideo ? (data.type > 0 ? true : false) : false
    ...
    public func provider(_ provider: CXProvider, perform action: CXSetHeldCallAction) {
        ...
        if (call.isOnHold != action.isOnHold) {
            call.isOnHold = action.isOnHold
            call.isMuted = action.isOnHold
//            self.setHold(call.data, onHold: action.isOnHold)
    ...
    public func provider(_ provider: CXProvider, perform action: CXPlayDTMFCallAction) {
        ...
        self.sendEvent(SwiftFlutterCallkitIncomingPlugin.ACTION_CALL_TOGGLE_DTMF, [ "id": action.callUUID.uuidString, "digits": action.digits ])
        action.fulfill()
    }
